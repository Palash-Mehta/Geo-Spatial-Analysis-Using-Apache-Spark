...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val finalDf = spark.sql("select rectangle,count(point) from joinResult group by rectangle order by rectangle asc")
    //joinDf.createOrReplaceTempView("joinResult")
    //val final_df=spark.sql(sqlText= "select rectangle,count(point) from joinResult group by rectangle")
    finalDf.createOrReplaceTempView("groupresult")
